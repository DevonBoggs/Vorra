...
export default function App() {
  ...
  // First-run AI disclaimer acknowledgment
  const [showAiAck, setShowAiAck] = useState(() => !localStorage.getItem('vorra-ai-ack'));
      ...
      <div style={{ display: "flex", height: "100vh", background: T.bg, color: T.text, fontFamily: "'Outfit','Inter',sans-serif", zoom: (data.uiZoom || 100) / 100 }}>
        ...
        {showAiAck && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(8px)' }}>
            <div className="pop-in" style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 20, padding: '36px 40px', maxWidth: 440, textAlign: 'center', boxShadow: `0 24px 60px rgba(0,0,0,.5)` }}>
              <div style={{ fontSize: fs(20), fontWeight: 800, marginBottom: 16, color: T.text }}>Welcome to Vorra</div>
              <div style={{ fontSize: fs(13), color: T.soft, lineHeight: 1.7, marginBottom: 12, textAlign: 'left' }}>
                Vorra uses AI to enrich your courses with study context {'\u2014'} topics, exam tips, competencies, and study schedules.
              </div>
              <div style={{ fontSize: fs(13), color: T.soft, lineHeight: 1.7, marginBottom: 16, textAlign: 'left' }}>
                This data is generated by AI models, not sourced from your school. Always cross-reference with your official course syllabus, student portal, and instructor guidance.
              </div>
              <div style={{ fontSize: fs(13), color: T.accent, fontWeight: 600, marginBottom: 24 }}>
                AI is your study accelerator. Your school docs are the source of truth.
              </div>
              <button onClick={() => { localStorage.setItem('vorra-ai-ack', '1'); setShowAiAck(false); }} className="sf-btn" style={{ padding: '12px 36px', borderRadius: 12, border: 'none', background: `linear-gradient(135deg,${T.accent},${T.accent}cc)`, color: '#060e09', fontSize: fs(14), fontWeight: 700, cursor: 'pointer' }}>Got it</button>
            </div>
          </div>
        )}
